...
|------|------|----------|
| `root` | 仅用于创建用户和授权 | **第一章** |
| `devops` | 日常登录服务器、系统管理（与 SQL Server 无关） | **第一章附** |
| `mssql_admin` | 专门管理 SQL Server，通过 sudo 执行特权操作 | **第二章 ~ 第十一章** |

> 每个章节标题后都标注了 `[root]`、`[mssql_admin]` 或 `[本地开发机]`，方便识别。
...
> **说明：本节与 SQL Server 安装无关。** 这是一个可选步骤，目的是创建一个日常登录服务器用的普通用户，避免平时使用 root 操作带来的误操作风险。如不需要可跳过。
...
# 配置 sudo 免密（可选）
echo 'devops ALL=(ALL) NOPASSWD: ALL' | tee -a /etc/sudoers.d/devops
...
|------|------|
| 日常登录服务器 | 用 `devops` 登录 |
| 需要管理 SQL Server | `su - mssql_admin` 切换 |
| 需要 root 权限 | 通过 `sudo` 执行，不直接登录 root |

---

## 二、系统环境准备 `[mssql_admin]`

> 从本章开始，所有命令均在 mssql_admin 用户下执行，需要特权的命令使用 `sudo`。
...
## 三、安装 SQL Server 2022 `[mssql_admin]`
...
```
[root]        1.  创建 mssql_admin 用户并授予 sudo 权限
[root]        2.  配置 sudo 免密（可选）
              3.  su - mssql_admin 切换用户
---------------------------------------------------------
[mssql_admin] 4.  修复 CentOS 8 软件源（sudo tee）
[mssql_admin] 5.  检查磁盘空间（>= 6GB）
[mssql_admin] 6.  处理 SELinux（sudo setenforce 0）
[mssql_admin] 7.  安装依赖（sudo dnf install）
[mssql_admin] 8.  配置 swap（sudo dd / sudo mkswap）
[mssql_admin] 9.  添加微软仓库 → 安装 mssql-server（sudo）
[mssql_admin] 10. 设置排序规则（sudo mssql-conf）
[mssql_admin] 11. 运行 mssql-conf setup 选择 Developer 版本（sudo）
[mssql_admin] 12. 限制内存为 768MB（sudo mssql-conf）
[mssql_admin] 13. 安装 sqlcmd 命令行工具（sudo dnf install）
[mssql_admin] 14. 配置环境变量（~/.bashrc，无需 sudo）
[mssql_admin] 15. 开放防火墙 1433 端口（sudo firewall-cmd）
[mssql_admin] 16. 创建日常开发用的数据库账户（sqlcmd，无需 sudo）
[mssql_admin] 17. 配置定时备份（crontab，无需 sudo）
```

> **原则**：root 只做第 1~2 步，其余全部由 mssql_admin 通过 sudo 完成。
> 不需要 sudo 的操作（如 sqlcmd 连接数据库、编辑 ~/.bashrc、crontab）直接以 mssql_admin 身份执行。
